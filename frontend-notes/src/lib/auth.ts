...
// notes 専用 UserPool(NotesAuthStack が作成 / CfnOutput NotesUserPoolId)。
// 本家 rou39.com / uraneko とは会員基盤を分離している。
const USER_POOL_ID = 'ap-northeast-1_sSHRq0nPz';

// notes 用 App Client ID(NotesAuthStack が作成 / CfnOutput NotesUserPoolClientId)。
// SPA の App Client ID はブラウザバンドルに露出する公開値(秘密ではない)なので、
// 本家/uraneko と同様にソースへ直書きする。これによりクリーンビルドでも再現性を保つ。
// NotesAuth スタックを作り直して ID が変わった場合のみ、この値を更新すること。
const CLIENT_ID = '36cuk9ghivhalv53u0rt92a5m0';

const COGNITO_DOMAIN = 'https://notes-auth.rou39.com';
...
// notes の認証は Google 連携のみ(email/password のネイティブ登録・ログインは存在しない)。
// uraneko の auth.ts から signUp / signIn / forgotPassword 系を意図的に落としている。
...
const AUTH_RETURN_TO_KEY = 'notes_auth_return_to';
